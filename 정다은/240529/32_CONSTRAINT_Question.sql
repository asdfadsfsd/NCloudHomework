...
CREATE TABLE PROFESSOR_PK(
	...
	ORDERS VARCHAR2(10),
	HIREDATE DATE,
	CONSTRAINT PK_PROFESSOR_P NO PRIMARY KEY(PNO)
);

--3) PF_TEMP 테이블에 PNO을 PK로 추가하세요.
ALTER TABLE PF_TEMP ADD CONSTRAINT PK_PF_TEMP PRIMARY KEY(PNO);

--4) COURSE_PROFESSOR 테이블에 CNO, PNO을 PK로 추가하세요.
ALTER TABLE COURSE_PROFESSOR ADD CONSTRAINT PK_COURSE_PROFESSOR PRIMARY KEY(CNO, PNO);
...
CREATE TABLE T_BOARD_FILE(
	...
	CONSTRAINT PK_BOARD_FILE PRIMARY KEY (BOARD_NO, BOARD_FILE_NO)
);
...
CREATE TABLE T_SNS_UPLOADED(
	SNS_NO NUMBER,
	SNS_UPL_NO NUMBER,
	CONSTRAINT PK_UPLOADED_NOS PRIMARY KEY (SNS_NO, SNS_UPL_NO),
	CONSTRAINT FK_UPLOADED_NOS FOREIGN KEY (SNS_NO)
		REFERENCES T_SNS(SNS_NO)
);