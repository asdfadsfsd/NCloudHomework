...
SELECT PNO
	 , PNAME
	 , ROUND((SYSDATE - TO_DATE(HIREDATE)) / 7)
	FROM PROFESSOR;
...
SELECT ENO,
       ENAME,
       FLOOR(MONTHS_BETWEEN(SYSDATE, HDATE) / 12) AS 년수,
       FLOOR(MOD(MONTHS_BETWEEN(SYSDATE, HDATE), 12)) AS 월수,
       SYSDATE - ADD_MONTHS(HDATE, FLOOR(MONTHS_BETWEEN(SYSDATE, HDATE))) AS 일수
FROM EMP;
...
SELECT SNO 
	 , SNAME || ' 학생의 평균 평점은 ' || TRUNC(AVR, 2) || '입니다.'
	FROM STUDENT;

--2) 교수의 부임일을 다음 형식으로 표현하세요
--'OOO 교수의 부임일은 YYYY년 MM월 DD일입니다.'
SELECT PNO
	 , PNAME || ' 교수의 부임일은 ' || SUBSTR(TO_CHAR(HIREDATE, 'YYYYMMDD'), 1, 4) || '년 ' ||
	 SUBSTR(TO_CHAR(HIREDATE, 'YYYYMMDD'), 5, 2) || '월 ' || SUBSTR(TO_CHAR(HIREDATE, 'YYYYMMDD'), 5, 2) || '일'||' 입니다'
	FROM PROFESSOR;
